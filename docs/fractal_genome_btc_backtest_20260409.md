...
## Cycle 6 Latest-Fold-First Search

이번 cycle은 `wf_1 우선`, `stress survival min/mean 동시 반영`, `leaf gene deviation penalty`, `persistent depth archive`를 묶어서 다시 돌렸다.

### Cycle 6 validation

- `py_compile`: 통과
- `verify_fractal_genome_cells.py --mode self-check`: 통과
- `verify_fractal_genome_cells.py --mode mock-openai`: 통과
- 메인 search 재실행: 완료
- verifier 재실행: 완료

### Cycle 6 main search outcome

- Command family: `scripts/search_pair_subset_fractal_genome.py`
- Outcome:
  - `selection.reason = no_gate_pass_near_frontier_structural`
  - `top_k_wf1_pass_count = 5`
  - `top_k_stress_pass_count = 0`
  - selected candidate: `tree_depth=2`, `logic_depth=1`, `leaf_cardinality=3`
  - `leaf_gene_penalty = 0.3833`

이번 selected candidate는 cycle 5의 `depth=3 / logic=3` 고수익 후보가 아니라, `latest fold`를 먼저 맞추는 더 보수적인 후보로 이동했다.

### Cycle 6 selected candidate vs embedded baseline

아래 비교는 search report 기준이고, 승률은 `daily_win_rate`다.

| Window | Candidate Return | Delta vs Baseline | Candidate Win Rate | Delta Win Rate | Candidate MDD | Delta MDD |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| Recent 2 months | `0.4491` | `-0.0109` | `0.2333` | `-0.1226` | `-0.088248` | `-0.000220` |
| Recent 6 months | `2.9997` | `+0.0129` | `0.3169` | `-0.1611` | `-0.090735` | `-0.002584` |
| Full history | `1743.8183` | `+155.6857` | `0.2744` | `-0.2383` | `-0.117984` | `+0.013224` |

읽는 법:

- `Delta MDD`는 양수일수록 baseline보다 더 얕다
- 이번 cycle의 selected candidate는 `2개월` 수익률은 소폭 낮고, `6개월 / 전체` 수익률은 baseline보다 높다
- 승률은 여전히 전 구간에서 baseline보다 낮다
- `전체 구간 MDD`는 baseline보다 더 얕아졌다

### Cycle 6 search robustness

| Metric | Cycle 5 | Cycle 6 |
| --- | ---: | ---: |
| Fold pass rate | `0.0000` | `1.0000` |
| Stress survival rate mean | `0.0000` | `0.3333` |
| Stress survival rate min | `n/a` | `0.3333` |
| wf_1 delta total return | `-0.0221` | `0.0000` |
| wf_1 delta MDD | `-0.0109` | `0.0000` |
| wf_1 delta daily win rate | `0.0000` | `0.0000` |
| Gate passed | `false` | `false` |

핵심은 이렇다.

- search가 더 이상 `wf_1`에서 바로 깨지는 공격적 후보를 선택하지 않는다
- selected candidate는 `wf_1`과 다른 robustness fold에서 baseline parity를 맞췄다
- 하지만 `commission 1.5x / 2.0x`에서 여전히 무너져서 `stress_survival_rate_mean = 0.3333`에 멈춘다
- 그래서 `robustness_gate_pass_count = 0`은 그대로다

### Cycle 6 verifier result
...
#### Verifier walk-forward summary

| Metric | Value |
| --- | ---: |
| Fold pass rate | `0.0000` |
| Stress survival rate mean | `0.0556` |
| Min fold stress survival rate | `0.0000` |
| Stress survival thresholds | `mean=0.67`, `min=0.67` |
| Promotion gate passed | `false` |

`wf_1` 기준으로 verifier selected candidate는 아래였다.

| Metric | Candidate | Baseline | Delta |
| --- | ---: | ---: | ---: |
| Total return | `0.3165` | `0.3998` | `-0.0833` |
| Daily win rate | `0.2034` | `0.2712` | `-0.0678` |
| MDD | `-0.090720` | `-0.091014` | `+0.000294` |
| Stress survival rate | `0.0000` | `-` | `-` |

즉 verifier는 여전히 `depth_1` 후보만 승인하고 있고, 그 후보는 최신 구간 수익률과 win rate에서 baseline보다 진다.

### Cycle 6 conclusion

- 이번 cycle의 진짜 진전은 `search가 최신 구간에서 바로 깨지는 후보를 버리고, wf_1 parity 후보를 고르기 시작했다`는 점이다
- `fold_pass_rate 1.0`과 `wf_1 delta 0.0`은 분명한 개선이다
- 하지만 `stress survival 0.3333`이 아직 한계고, verifier는 여전히 `depth_1`에서 막힌다
- 즉, 병목은 이제 더 명확하다:
  - `stress survival 0.67 이상`
  - `verifier depth_2 stage의 성과 열세 제거`
  - `승률 열세를 유지한 채 수익률만 개선하는 패턴 탈피`

## Cycle 7

이번 cycle은 목표 2개를 직접 겨냥했다.

- search:
  - `cost reserve` 기준을 `0.67`로 끌어올리고, `latest_non_nominal_stress_reserve_score`와 `trade_count_ratio` 벌점을 크게 강화했다.
  - 상위 후보에 대해 `mild / strong / ultra / max_defense` leaf gene 방어형 변형을 자동 평가하도록 넣었다.
- verifier:
  - 각 stage가 실제 `min_tree_depth` 이상 후보만 선택하도록 고쳤다.
  - `search_seed_summary` 인자를 추가해 메인 search가 찾은 깊은 후보를 verifier stage population에 주입할 수 있게 했다.

### Cycle 7 search result

메인 search는 깊은 구조를 유지했다.

- `selection.reason = progressive_near_frontier_structural_pass`
- `winner_tree_depth = 4`
- `winner_logic_depth = 4`
- `top_k_wf1_pass_count = 12`
- `top_k_stress_pass_count = 0`
- `top_k_cost_reserve_pass_count = 0`

#### Search stress summary

| Metric | Cycle 6 | Cycle 7 |
| --- | ---: | ---: |
| Stress survival rate mean | `0.3333` | `0.3333` |
| Stress survival rate min | `0.3333` | `0.3333` |
| Latest 2.0x delta total return | `-0.0316` | `-0.0090` |
| Latest 2.0x delta daily win rate | `-0.0167` | `-0.0167` |
| Latest 2.0x delta MDD | `+0.0037` | `-0.0013` |
| Latest stress reserve score | `-779.3` | `-588.4` |

읽는 법:

- `stress survival 0.67`은 아직 못 넘었다
- 하지만 `2.0x commission` 기준 손실은 `-3.16% -> -0.90%`로 줄었다
- 즉 `비용 민감도는 완화`됐지만 `pass threshold`까지는 아직 부족하다

### Cycle 7 verifier result

stage depth 강제와 search seed 주입 이후 verifier는 실제 `depth_2+` 후보를 뽑아 평가했다.

- unseeded:
  - `selection.reason = no_stage_pass`
  - `best_stage = null`
- seeded:
  - `selection.reason = no_stage_pass`
  - `best_stage = null`
  - 하지만 `depth_2 / depth_3 / depth_4` 모두 실제 깊은 후보로 평가됨

seeded verifier에서 깊은 후보의 최신 구간 비교는 아래였다.

| Window | Candidate Return Delta | Candidate Win Rate Delta | Candidate MDD Delta |
| --- | ---: | ---: | ---: |
| Recent 2 months | `+0.0164` | `-0.0169` | `0.0000` |
| Recent 6 months | `+0.3692` | `-0.0055` | `-0.0058` |

핵심 해석:

- `depth_2+` 후보가 최신 2개월/6개월 `수익률`은 baseline을 넘기기 시작했다
- 하지만 `daily_win_rate`는 최근 2개월 기준 정확히 `1일 차이` 수준으로 아직 모자란다
- 그래서 verifier gate는 여전히 `recent_2m_baseline_gate_failed`에서 막힌다

### Cycle 7 conclusion

- 목표 1 `stress survival >= 0.67`:
  - 아직 실패
  - 다만 `2.0x` 비용 구간 손실 폭은 크게 줄였다
- 목표 2 `depth_2+ candidate keeps latest-window return / win rate`:
  - 절반 달성
  - `depth_2+` 후보가 최신 구간 `수익률`은 지키기 시작했다
  - `승률`은 최근 2개월 기준 `-0.0169`로 아직 미달

현재 병목은 하나로 좁혀졌다.

- `latest-window daily_win_rate`를 `1일 차이` 이상 개선하면서
- 동시에 `2.0x` 비용 구간에서 total return을 0 위로 올려야 한다
