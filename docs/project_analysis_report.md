...
> 참고: 기존 Streamlit UI는 제거되었고, 현재 운영 경로는 CLI와 Telegram 제어 기준입니다.
...
│   ├── rotation_target_050_live.py # 실운영/데모 트레이더 실행기
│   └── telegram_bot.py             # Telegram 제어 봇
├── data/
...
### A. scripts/gp_crypto_evolution.py - 핵심 진화 엔진
...
### B. scripts/gp_crypto_infer.py - 모델 추론 & 분석 CLI
...
### C. scripts/backtest_short_vol_breakout.py - 대안 전략
...
### D. liquidation_monitor.py - 실시간 청산 레이더
