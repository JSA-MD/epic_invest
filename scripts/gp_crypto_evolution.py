...
def fetch_klines(symbol: str, interval: str,
    ...
    """Fetch OHLCV klines from Binance Futures API with pagination."""
...
def enrich_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return base[list(BASIC_FEATURES)]


def load_pair(symbol: str, interval: str = TIMEFRAME,
              start: str = TRAIN_START, end: str = TEST_END) -> pd.DataFrame:
    """Load pair data from cache or fetch from API."""
    DATA_DIR.mkdir(parents=True, exist_ok=True)
    cache_file = DATA_DIR / f"{symbol}_{interval}_{start}_{end}.csv"
...
def load_all_pairs(pairs: List[str] = PAIRS) -> pd.DataFrame:
    """Load all pairs and merge into wide DataFrame."""
    print(f"Loading {len(pairs)} crypto pairs...")
    dfs = [load_pair(pair) for pair in pairs]
